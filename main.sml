...
(*	 "let val C = nth(children,"^(int2str (length passed))^")in fn () => let val X_"^h^" V = execute C in V end end" *)
	 "let val C = nth(children,"^(int2str (length passed))^")in fn () => case execute C of X_"^h^" V => V | _ => raise InternalError \"S4.3\" end"
	 )::(DOtokens (h::passed,t))
      end
      | DOtokens _ = []
...
(* Unit match trees *)
    ...
    fun leafs (Leaf _) = 1
      | leafs (Tree(_,cs)) = List.foldl (op +) 0 (map leafs cs)
	...
	(emit ("(" ^ (int2str n0) ^ "," ^ (int2str (leafs p)) ^ ")");
	 ...
	 emit (int2str (n+1));
	 emit ",0) in ((app (fn(r,m)=>Array.update (a,r,m)) matchcounts); a) end\n\n\
...
(* Unit rules *)
    ...
    fun closurize unitrules =
      let fun member (a,nil) = false | member (a,h::t) = if a=h then true else member (a,t)
	  val initials =
	    accum
...
(* Main *)
    ...
    fun fatal s = TextIO.output (TextIO.stdOut,"Fatal error: "^s^"\n")

    fun main inputfilename =
      let
	val outputfilename = inputfilename ^ ".sml"
	val (inputf, outputf) = (TextIO.openIn inputfilename,TextIO.openOut outputfilename)
	...
	emit "fun getreplacement (XXXrewrite t) = t | getreplacement _ = raise InternalError \"problem with rewrite 996\"\n\
	\  end\n\
