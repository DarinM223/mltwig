...
struct
  ...
  struct
    ...
    fun int2str (i: int) =
      if i < 0 then ("-" ^ (makestring (0 - i))) else makestring i

    val next_label = ref 0
    fun newLabel () =
      (inc next_label; "_Label" ^ (int2str (!next_label - 1)))
    ...
    struct
      datatype extension =
        Ext of {name: string, usage: int ref, modif: bool ref}
      ...
      val SPregister = ref
        (Ext {name = "r15", usage = ref 1, modif = ref false})
      ...
      fun calling () =
        (case !rsr of
         ...
         | REFERS r =>
             let
               val Ext {usage = u, ...} = R0register
             in
               if !u <> 0 then
                 let
                   val newreg as Ext {name = s, ...} = choose registers
                 in
      ...
      fun calcula ((Ext {modif = ref true, ...}) :: t, n, ac: int) =
            calcula (t, n + n, ac + n)
        | calcula ((Ext {modif = ref false, ...}) :: t, n, ac) =
            calcula (t, n + n, ac)
    ...
    fun givereg (tree (_, _, _, {regre = rr as ref (register r), ...})) =
          (decreaseusage r; if (usage r) = 0 then rr := ghostregister r else ())
    ...
    fun getreg (tree (_, _, _, {regre = rr as ref (register r), ...})) =
          rr := register (allocate ())
      | getreg (tree (_, _, _, {regre = r, ...})) =
          r := register (allocate ())
    ...
    fun count_arguments (tree (ARG, [_, t], _, _)) =
          1 + (count_arguments t)
      | count_arguments _ = 0
    ...
    fun get_sizecode (tree (_, _, _, {sizcod = ref (unaryfun f), ...})) = f
      | get_sizecode _ =
          fatal "no attribute sizcod = ref unaryfun"

    fun get_sizecodecvt (tree (_, _, _, {sizcod = ref (binaryfun f), ...})) = f
      | get_sizecodecvt _ =
          fatal "no attribute sizcod = ref binaryfun"
    ...
    *)
    ...
    fun cost_plus
      ((s, t, f, e, d, m, h, v, i): cost, (s2, t2, f2, e2, d2, m2, h2, v2, i2)) =
      (s + s2, t + t2, false, f orelse f2, false, 0, 0, ref false, 0)

    fun sum_costs [] = zerocost
      | sum_costs (h :: t) =
          List.foldl (fn (a, b) => cost_plus (a, b)) h t

    fun cost_less
      ((s1, t1, _, _, _, _, _, _, _): cost, (s2, t2, _, _, _, _, _, _, _): cost) =
      (s1 + t1) < (s2 + t2)

    fun C (s, t, f, e, d, m) =
      (s, t, f, e, d, m, 0, ref false, 0)
    ...
    fun emitname t =
      ...
      | LABEL =>
          ( emit ((get_label o child1 o child0) t)
          ; emit "+"
    ...
    fun emitlocation t =
      ...
      | numbered 2 =>
          ( emitname (child1 t)
          ; emit "("
          ; emit ((reg2str o get_reg o child2) t)
          ; emit ")"
          ; (givereg o child2) t
          )
      | numbered 3 =>
          ( emitname (child2 t)
          ; emit "("
          ; emit ((reg2str o get_reg o child1) t)
          ; emit ")"
          ; (givereg o child1) t
          )
      | numbered 4 =>
          ( emit "*"
          ; emitname (child1 (child0 t))
          ; emit "("
          ; emit ((reg2str o get_reg o child2) (child0 t))
          ; emit ")"
          ; (givereg o child2) (child0 t)
          )
      | numbered 5 =>
          ( emit "*"
          ; emitname (child2 (child0 t))
          ; emit "("
          ; emit ((reg2str o get_reg o child1) (child0 t))
          ; emit ")"
    ...
    fun emitioperand t =
      case get_kind t of
        numbered 11 =>
          ( emitlocation (child1 t)
          ; emit "["
          ; emit ((reg2str o get_reg o child1 o child2) t)
          ; emit "]"
      ...
      | numbered 13 =>
          ( emitlocation (child2 t)
          ; emit "["
          ; emit ((reg2str o get_reg o child2 o child2) t)
          ; emit "]"
      ...
      in
        case kind of
          numbered 31 =>
            ...
            ; emit "3\t"
            ; ((emitoperand o child2) t)
            ; emit ","
            ; (emitoperand o child1) t
            ; emit ","
            )
        | numbered 32 =>
            ( emit ((vaxop o Op o get_op o child0) t)
            ; emit (sizecode (get_size t))
            ; emit "\t"
            ; emitoperand (child1 t)
            ; emit ","
            )
        | numbered 33 =>
            let
              val (sf, st) = get_sizecodecvt (child0 t)
            in
              emit "cvt";
              emit (sf ((get_size o child1) t));
              emit (st (get_size t));
              emit "\t";
              (emitoperand o child1) t;
              emit ","
        ...
        | numbered 37 =>
            ( emit "extzv\t$"
            ; emit ((int2str o get_ivalue o child2) t)
            ; emit ",$"
            ; emit (int2str (32 - ((get_ivalue o child2) t)))
            ; emit ","
            ; emit ((reg2str o get_reg o child1) t)
            ; emit ","
            ; (givereg o child1) t
        ...
        | numbered k =>
            (if 1 <= k andalso k <= 8 then
               (emit "movab\t"; emitlocation t; emit ",")
             else if 11 <= k andalso k <= 15 then
               ( emit "mova"
               ; emit ((sizecode o sizeioperand) t)
               ; emit "\t"
               ; emitioperand t
               ; emit ","
               )
             else if 21 <= k andalso k <= 27 then
               ( emit "mov"
               ; emit (sizecode (get_size t))
               ; emit "\t"
               ; emitoperand t
               ...
               (emit "# strange kind of computation"))
        | _ => ()
      end
    ...
    | X_ntest of unit
    | X_test of unit
    | X_computation of unit
    | X_destination of unit
    | X_ioperand of unit
    | X_location of unit
    | X_operand of unit
    | X_reg of unit
  ...
  struct
    ...
    fun execute_cost (n: rule, ir, children) =
      ...
      in
        case n of
          86 =>
            (case map cost children of
               [stm, reg] =>
                 ((WDH1
                     ( DC
                     , 0
                     , 0
                     , get_cost_setFlags reg
                     , false
                     , get_cost_dontDestroy reg
                     , 0
                     )))
             | _ => raise InternalError "S4")
        | 85 =>
            (case map cost children of
               [] => ((if (get_ivalue ir) <> 0 then ABORT () else DC))
             | _ => raise InternalError "S4")
        | 84 => DC
        | 83 =>
            (case map cost children of
               [operand, args] => ((WD (DC, 1, 6, false, false, false, 0)))
             | _ => raise InternalError "S4")
        | 82 =>
            (case map cost children of
               [args, location] => ((WD (DC, 6, 20, false, false, false, 0)))
             | _ => raise InternalError "S4")
        | 81 =>
            (case map cost children of
               [result] => ((WD (DC, 2, 2, false, false, false, 0)))
             | _ => raise InternalError "S4")
        | 80 =>
            (case map cost children of
               [reg] => ((WD (DC, 2, 2, false, false, false, 0)))
             | _ => raise InternalError "S4")
        | 79 => DC
        | 78 =>
            (case map cost children of
               [reg] =>
                 ((if
                     not
                       ((get_ivalue (get_subtree (3, ir))) = 1
                        orelse (get_ivalue (get_subtree (3, ir))) = ~1)
                     orelse get_cost_dontDestroy reg
                   then ABORT ()
        ...
        | 77 =>
            (case map cost children of
               [binop, operand, reg] =>
                 ((if
                     get_cost_dontDestroy reg
                     orelse not ((commutative o get_op) (get_subtree (1, ir)))
                   then ABORT ()
        ...
        | 76 =>
            (case map cost children of
               [binop, reg, operand] =>
                 ((if get_cost_dontDestroy reg then ABORT ()
                   else WDH1 (DC, 2, 0, true, false, false, 0)))
             | _ => raise InternalError "S4")
        | 75 =>
            (case map cost children of
               [destination, zero] => ((WD (DC, 1, 4, true, true, false, 0)))
             | _ => raise InternalError "S4")
        | 74 =>
            (case map cost children of
               [destination, operand] =>
                 ((if
                     not
                       ((equal ((get_subtree (1, ir)), (get_subtree
                           (2, (get_subtree (2, ir))))))
                        andalso
                        ((get_ivalue (get_subtree (3, (get_subtree (2, ir)))))
                         = 1
                         orelse
                         (get_ivalue (get_subtree (3, (get_subtree (2, ir)))))
                         = ~1))
                   then
                     ...
                     WD
                       ( DC
                       , 1 - (get_cost_space operand)
                       , 8 - (get_cost_time operand)
                       , true
                       , true
                       , false
                       , 0
        ...
        | 73 =>
            (case map cost children of
               [destination, binop, operand1, operand2] =>
                 ((if
                     not ((commutative o get_op) (get_subtree
                       (1, (get_subtree (2, ir))))) orelse (get_sideEffect DC)
                     orelse
                     not (equal ((get_subtree (1, ir)), (get_subtree
                       (3, (get_subtree (2, ir))))))
                   then
                     ABORT ()
                   else
                     WD
                       ( DC
                       , 1 - (get_cost_space operand2)
                       , 8 - (get_cost_time operand2)
                       , true
                       , true
                       , false
                       , 0
               ...
               [destination, binop, operand1, operand2] =>
                 ((if
                   ...
                   then
                     ...
                     WD
                       ( DC
                       , 1 - (get_cost_space operand1)
                       , 8 - (get_cost_time operand1)
                       , true
                       , true
                       , false
                       , 0
        ...
        | 70 =>
            (case map cost children of
               [destination, reg] => ((WDH1 (DC, 2, 12, true, true, false, 0)))
             | _ => raise InternalError "S4")
        | 69 =>
            (case map cost children of
               [] => ((C (4, 0, false, false, false, 0)))
             | _ => raise InternalError "S4")
        | 68 =>
            (case map cost children of
               [] => ((C (4, 0, false, false, false, 0)))
            ...
            (case map cost children of
               [] =>
                 ((let
                   ...
                   in
                     C ( if i >= 0 then
                           if i > 32767 then 4 else if i > 127 then 2 else 1
        ...
        | 49 =>
            (case map cost children of
               [] => ((C (0, 100, false, false, false, 0)))
             | _ => raise InternalError "S4")
        | 48 =>
            (case map cost children of
               [] => ((C (0, 16, false, false, false, 0)))
             | _ => raise InternalError "S4")
        | 47 =>
            (case map cost children of
               [] => ((C (0, 4, false, false, false, 0)))
        ...
        | 43 =>
            (case map cost children of
               [test] => ((WDH1 (DC, 9, 12, true, false, false, 0)))
             | _ => raise InternalError "S4")
        | 42 =>
            (case map cost children of
               [relop, stm, zero] =>
        ...
        | 40 =>
            (case map cost children of
               [relop, operand1, operand2] =>
                 ((WD (DC, 1, 4, false, false, false, 0)))
             | _ => raise InternalError "S4")
        | 39 =>
            (case map cost children of
               [flag] => ((WD (DC, 4, 8, false, false, false, 0)))
             | _ => raise InternalError "S4")
        | 38 =>
            (case map cost children of
               [flag] => ((WD (DC, 4, 8, false, false, false, 0)))
        ...
        | 32 =>
            (case map cost children of
               [location] => ((WDHA (DC, 1, 4, false, false, false, 0)))
             | _ => raise InternalError "S4")
        | 31 =>
            (case map cost children of
               [cvtop, operand] => ((WDHA (DC, 1, 0, false, false, false, 0)))
             | _ => raise InternalError "S4")
        | 30 =>
            (case map cost children of
               [unop, operand] => ((WDHA (DC, 1, 0, false, false, false, 0)))
             | _ => raise InternalError "S4")
        | 29 =>
            (case map cost children of
               [binop, operand1, operand2] =>
        ...
        | 28 =>
            (case map cost children of
               [operand] =>
                 ((if
                     (get_size ir) > 8
                     orelse ((((sizecode o get_size) ir) = "X"))
                   then
                     ABORT ()
                   else
                     WDHA (DC, 1, 4, get_cost_setFlags operand, false, false, 0)))
             | _ => raise InternalError "S4")
        | 27 =>
            (case map cost children of
               [destination] => ((WDHA (DC, 0, 0, false, false, false, 0)))
             | _ => raise InternalError "S4")
        | 26 =>
            (case map cost children of
               [] =>
                 ((if not ((get_ivalue ir) >= 0 andalso (get_ivalue ir) < 64) then
                     ABORT ()
                   else
        ...
        | 25 =>
            (case map cost children of
               [name] => ((WD (DC, 1, 0, false, false, false, 0)))
             | _ => raise InternalError "S4")
        | 24 =>
            (case map cost children of
               [reg] =>
        ...
        | 23 =>
            (case map cost children of
               [ioperand] =>
                 ((if (get_size ir) <> (get_cost_isize ioperand) then ABORT ()
                   else WDHA (DC, 0, 4, false, false, false, 0)))
             | _ => raise InternalError "S4")
        | 22 =>
            (case map cost children of
               [location] => ((WDHA (DC, 0, 4, false, false, false, 0)))
             | _ => raise InternalError "S4")
        | 21 =>
            (case map cost children of
               [reg, location] =>
            ...
            (case map cost children of
               [location, reg] =>
                 ((WDHAs
                     ...
                     , false
                     , 0
                     , get_ivalue (get_subtree (3, (get_subtree (3, ir))))
                     )))
             | _ => raise InternalError "S4")
        | 17 =>
            (case map cost children of
        ...
        | 16 =>
            (case map cost children of
               [reg, name] => ((WDHA (DC, 1, 4, false, false, false, 0)))
             | _ => raise InternalError "S4")
        | 15 =>
            (case map cost children of
               [name, reg] => ((WDHA (DC, 1, 4, false, false, false, 0)))
             | _ => raise InternalError "S4")
        | 14 =>
            (case map cost children of
               [reg, name] => ((WDHA (DC, 1, 0, false, false, false, 0)))
             | _ => raise InternalError "S4")
        | 13 =>
            (case map cost children of
               [name, reg] => ((WDHA (DC, 1, 0, false, false, false, 0)))
             | _ => raise InternalError "S4")
        | 12 =>
            (case map cost children of
               [reg] => ((WDHA (DC, 1, 0, false, false, false, 0)))
             | _ => raise InternalError "S4")
        | 11 =>
            (case map cost children of
        ...
        | 10 =>
            (case map cost children of
               [computation] => ((WD (DC, 1, 0, true, true, true, 0)))
             | _ => raise InternalError "S4")
        | 9 =>
            (case map cost children of
               [computation] => ((WDH1 (DC, 1, 0, true, false, false, 0)))
             | _ => raise InternalError "S4")
        | 8 => DC
        | 7 => DC
        | 6 =>
            (case map cost children of
               [ntest, location] => ((WD (DC, 1, 4, false, true, false, 0)))
             | _ => raise InternalError "S4")
        | 5 =>
            (case map cost children of
               [test] => ((WD (DC, 0, 0, false, true, false, 0)))
             | _ => raise InternalError "S4")
        | 4 =>
            (case map cost children of
               [location] => ((WD (DC, 1, 8, false, true, false, 0)))
             | _ => raise InternalError "S4")
        | 3 =>
            (case map cost children of
               [name] => ((WD (DC, 3, 8, false, true, false, 0)))
             | _ => raise InternalError "S4")
        | 2 => DC
        | 1 => DC
        | 0 => DC
    ...
    fun execute (Skeleton (n, _, ir, children)) =
      ...
      in
        case n of
          86 =>
            X_reg
              (case (execute (nth (children, 0)), execute (nth (children, 1))) of
                 (X_stm stm, X_reg reg) =>
        ...
        | 82 =>
            X_result
              (case (execute (nth (children, 0)), execute (nth (children, 1))) of
                 (X_args args, X_location location) =>
                   ( calling ()
                   ; emit "calls\t$"
                   ; emit ((int2str o count_arguments) (get_subtree (1, ir)))
                   ; emit ","
                   ; emitlocation (get_subtree (2, ir))
                   ; emit "\n"
                   )
               | _ => raise InternalError "S5")
        | 81 =>
            X_reg
              (case (execute (nth (children, 0))) of
                 (X_result result) => (set_reg (ir, resultReg ()))
               | _ => raise InternalError "S5")
        | 80 =>
            X_stm
              (case (execute (nth (children, 0))) of
                 (X_reg reg) =>
                   ( emit "movl\t"
                   ; emit (reg2str (get_reg (get_subtree (1, ir))))
                   ; emit ",r0\nret\n"
                   )
               | _ => raise InternalError "S5")
        | 79 =>
            ...
            X_reg
              (case (execute (nth (children, 0))) of
                 (X_reg reg) =>
                   ...
                   ; givereg (get_subtree (2, ir))
                   ; emit
                       (if (get_ivalue (get_subtree (3, ir))) > 0 then "inc"
                        else "dec")
                   ; emit (sizecode (get_size ir))
                   ; emit "\t"
                   ; emit (reg2str (get_reg ir))
                   ; emit "\n"
                   )
               | _ => raise InternalError "S5")
        | 77 =>
            X_reg
              (case
                 ( execute (nth (children, 0))
                 , execute (nth (children, 1))
                 , execute (nth (children, 2))
                 )
               of
                 (X_binop binop, X_operand operand, X_reg reg) =>
                   ( emit ((vaxop o Op o get_op) (get_subtree (1, ir)))
                   ; emit (get_sizecode (get_subtree (1, ir)) (get_size ir))
                   ; emit "2\t"
                   ; emitoperand (get_subtree (2, ir))
                   ; emit ","
                   ; emit (reg2str (get_reg (get_subtree (3, ir))))
                   ; emit "\n"
                   ; set_reg (ir, get_reg (get_subtree (3, ir)))
                   ; givereg (get_subtree (3, ir))
                   )
               | _ => raise InternalError "S5")
        | 76 =>
            X_reg
              (case
               ...
               of
                 (X_binop binop, X_reg reg, X_operand operand) =>
                   ( set_reg (ir, get_reg (get_subtree (2, ir)))
                   ; givereg (get_subtree (2, ir))
                   ; emit ((vaxop o Op o get_op) (get_subtree (1, ir)))
                   ; emit (get_sizecode (get_subtree (1, ir)) (get_size ir))
                   ; emit "2\t"
                   ; emitoperand (get_subtree (3, ir))
                   ; emit ","
                   ; emit (reg2str (get_reg ir))
                   ; emit "\n"
                   )
        ...
        | 75 =>
            X_stm
              (case (execute (nth (children, 0)), execute (nth (children, 1))) of
                 (X_destination destination, X_zero zero) =>
                   ( emit "clr"
                   ; emit (sizecode (get_size (get_subtree (1, ir))))
                   ; emit "\t"
                   ; emitoperand (get_subtree (1, ir))
                   ; emit "\n"
                   )
        ...
        | 73 =>
            X_stm
              (case
               ...
               of
                 (DOdestination, DObinop, DOoperand1, DOoperand2) =>
                   ( DOdestination ()
                   ; DObinop ()
                   ; DOoperand1 ()
                   ; emit ((vaxop o Op o get_op) (get_subtree
                       (1, (get_subtree (2, ir)))))
                   ; emit
                       (get_sizecode (get_subtree (1, (get_subtree (2, ir))))
                          (get_size ir))
                   ; emit "2\t"
                   ; emitoperand (get_subtree (2, (get_subtree (2, ir))))
                   ; emit ","
                   ; emitoperand (get_subtree (1, ir))
                   ; emit "\n"
                   ))
                 ...
                 (DOdestination, DObinop, DOoperand1, DOoperand2) =>
                   ...
                   ; emit
                       (get_sizecode (get_subtree (1, (get_subtree (2, ir))))
                          (get_size ir))
                   ; emit "2\t"
                   ; emitoperand (get_subtree (3, (get_subtree (2, ir))))
                   ; emit ","
                   ; emitoperand (get_subtree (1, ir))
                   ; emit "\n"
                   ))
        | 71 =>
            X_stm
              (case (execute (nth (children, 0)), execute (nth (children, 1))) of
                 (X_destination destination, X_computation computation) =>
                   ( emitcomputation (get_subtree (2, ir))
                   ; emitoperand (get_subtree (1, ir))
                   ; emit "\n"
                   )
               | _ => raise InternalError "S5")
        | 70 =>
            X_reg
              (case (execute (nth (children, 0)), execute (nth (children, 1))) of
                 (X_destination destination, X_reg reg) =>
                   ( set_reg (ir, get_reg (get_subtree (2, ir)))
                   ; givereg (get_subtree (2, ir))
                   ; emit "mov"
                   ; emit ((sizecode o get_size) (get_subtree (2, ir)))
                   ; emit "\t"
                   ; emit (reg2str (get_reg ir))
                   ; emit ","
                   ; emitoperand (get_subtree (1, ir))
        ...
        | 42 =>
            X_flag
              (case
               ...
               of
                 (X_relop relop, X_stm stm, X_zero zero) =>
                   (set_kind (ir, get_op (get_subtree (1, ir))))
               | _ => raise InternalError "S5")
        | 41 =>
            X_flag
              (case
               ...
               of
                 (X_relop relop, X_operand operand, X_zero zero) =>
                   ( emit "tst"
                   ; emit (get_sizecode (get_subtree (1, ir)) (get_size
                       (get_subtree (2, ir))))
                   ; emit "\t"
                   ; emitoperand (get_subtree (2, ir))
                   ; emit "\n"
        ...
        | 40 =>
            X_flag
              (case
               ...
               of
                 (X_relop relop, X_operand operand1, X_operand operand2) =>
                   ( emit "cmp"
                   ; emit (get_sizecode (get_subtree (1, ir)) (get_size
                       (get_subtree (2, ir))))
                   ; emit "\t"
        ...
        | 39 =>
            X_test
              (case (execute (nth (children, 0))) of
                 (X_flag flag) =>
                   ( emit "j"
                   ; emit ((vaxop o get_kind) ir)
                   ; emit "\t"
                   ; emit (get_truebranch ir)
                   ; emit "\n"
                   )
               | _ => raise InternalError "S5")
        | 38 =>
            X_ntest
              (case (execute (nth (children, 0))) of
                 (X_flag flag) =>
                   ( emit "j"
                   ; emit ((vaxop o negate o get_kind) ir)
                   ; emit "\t"
                   ; emit (get_truebranch ir)
                   ; emit "\n"
                   )
        ...
        | 37 =>
            X_ntest
              (case
               ...
               of
                 (DOtest) =>
                   ( set_truebranch ((get_subtree (1, ir)), get_truebranch ir)
                   ; DOtest ()
                   ; ()
                   ))
        | 36 =>
        ...
        | 31 =>
            X_computation
              (case (execute (nth (children, 0)), execute (nth (children, 1))) of
                 (X_cvtop cvtop, X_operand operand) => (set_kind_n (ir, 33))
               | _ => raise InternalError "S5")
        | 30 =>
            X_computation
              (case (execute (nth (children, 0)), execute (nth (children, 1))) of
                 (X_unop unop, X_operand operand) => (set_kind_n (ir, 32))
               | _ => raise InternalError "S5")
        | 29 =>
            X_computation
              (case
               ...
               of
                 (X_binop binop, X_operand operand1, X_operand operand2) =>
                   (set_kind_n (ir, 31))
               | _ => raise InternalError "S5")
        | 28 =>
            X_computation
        ...
        | 27 =>
            X_operand
              (case (execute (nth (children, 0))) of
                 (X_destination destination) => ()
               | _ => raise InternalError "S5")
        | 26 => X_operand (set_kind_n (ir, 22))
        | 25 =>
            X_operand
              (case (execute (nth (children, 0))) of
                 (X_name name) => (set_kind_n (ir, 22))
               | _ => raise InternalError "S5")
        | 24 =>
            X_operand
              (case (execute (nth (children, 0))) of
                 (X_reg reg) => (set_kind_n (ir, 21))
               | _ => raise InternalError "S5")
        | 23 =>
            X_destination
              (case (execute (nth (children, 0))) of
                 (X_ioperand ioperand) => (set_kind_n (ir, 25))
               | _ => raise InternalError "S5")
        | 22 =>
            X_destination
              (case (execute (nth (children, 0))) of
                 (X_location location) => (set_kind_n (ir, 24))
               | _ => raise InternalError "S5")
        | 21 =>
            X_ioperand
              (case (execute (nth (children, 0)), execute (nth (children, 1))) of
                 (X_reg reg, X_location location) => (set_kind_n (ir, 14))
               | _ => raise InternalError "S5")
        | 20 =>
            X_ioperand
              (case (execute (nth (children, 0)), execute (nth (children, 1))) of
                 (X_reg reg, X_location location) => (set_kind_n (ir, 13))
               | _ => raise InternalError "S5")
        | 19 =>
            X_ioperand
              (case (execute (nth (children, 0)), execute (nth (children, 1))) of
                 (X_location location, X_reg reg) => (set_kind_n (ir, 12))
               | _ => raise InternalError "S5")
        | 18 =>
            X_ioperand
              (case (execute (nth (children, 0)), execute (nth (children, 1))) of
                 (X_location location, X_reg reg) => (set_kind_n (ir, 11))
               | _ => raise InternalError "S5")
        | 17 =>
            X_location
              (case (execute (nth (children, 0))) of
                 (X_name name) => (set_kind_n (ir, 6))
               | _ => raise InternalError "S5")
        | 16 =>
            X_location
              (case (execute (nth (children, 0)), execute (nth (children, 1))) of
                 (X_reg reg, X_name name) => (set_kind_n (ir, 5))
               | _ => raise InternalError "S5")
        | 15 =>
            X_location
              (case (execute (nth (children, 0)), execute (nth (children, 1))) of
                 (X_name name, X_reg reg) => (set_kind_n (ir, 4))
               | _ => raise InternalError "S5")
        | 14 =>
            X_location
              (case (execute (nth (children, 0)), execute (nth (children, 1))) of
                 (X_reg reg, X_name name) => (set_kind_n (ir, 3))
               | _ => raise InternalError "S5")
        | 13 =>
            X_location
              (case (execute (nth (children, 0)), execute (nth (children, 1))) of
                 (X_name name, X_reg reg) => (set_kind_n (ir, 2))
               | _ => raise InternalError "S5")
        | 12 =>
            X_location
              (case (execute (nth (children, 0))) of
                 (X_reg reg) => (set_kind_n (ir, 1))
               | _ => raise InternalError "S5")
        | 11 => X_reg (set_reg (ir, get_temp ir))
        | 10 =>
            X_reg
              (case (execute (nth (children, 0))) of
                 (X_computation computation) =>
                   ( emitcomputation (get_subtree (2, ir))
                   ; set_reg (ir, get_temp (get_subtree (1, ir)))
                   ; emit (reg2str (get_temp (get_subtree (1, ir))))
                   ; emit "\n"
                   )
               | _ => raise InternalError "S5")
        | 9 =>
        ...
        | 7 =>
            X_stm
              (case (execute (nth (children, 0))) of
                 (X_operand operand) =>
                   (emit "# throw away "; emitoperand ir; emit "\n")
               | _ => raise InternalError "S5")
        | 6 =>
            X_stm
              (case
        ...
        | 5 =>
            X_stm
              (case
               ...
               of
                 (DOtest) =>
                   ( set_truebranch ((get_subtree (1, ir)), get_label
                       (get_subtree (2, ir)))
                   ; DOtest ()
                   ; ()
                   ))
        | 4 =>
            X_stm
              (case (execute (nth (children, 0))) of
                 (X_location location) =>
                   (emit "jmp\t"; emitlocation (get_subtree (1, ir)); emit "\n")
               | _ => raise InternalError "S5")
        | 3 =>
            X_stm
              (case (execute (nth (children, 0))) of
                 (X_name name) =>
                   (emit "jbr\t"; emitname (get_subtree (1, ir)); emit "\n")
               | _ => raise InternalError "S5")
        | 2 => X_stm (emit (get_label ir); emit ":\n")
        | 1 =>
            X_stm
              (case (execute (nth (children, 0)), execute (nth (children, 1))) of
                 (X_stm stm1, X_stm stm2) => ()
               | _ => raise InternalError "S5")
        | 0 =>
            X_body
              (case
    ...
    val matchtable =
      let val a = Array.array (87, 0)
      in ((app (fn (r, m) => Array.update (a, r, m)) matchcounts); a)
      end
       ...
       | XXXresult =>
           [Chain
              ...
              , [ Chain
                    ( 24
                    , XXXoperand
                    , [Chain (28, XXXcomputation, []), Chain (7, XXXstm, [])]
                    )
                , Chain (12, XXXlocation, [Chain (32, XXXcomputation, [])])
                ]
              )]
       | NAME =>
           [Chain
              ...
              , [ Chain
                    ...
                    , [ Chain
                          ( 28
                          , XXXcomputation
                          , [Chain (9, XXXreg, [Chain (12, XXXlocation, [])])]
                          )
                      , Chain (7, XXXstm, [])
                      ]
                    )
                , Chain
                    ...
                    , [Chain
                         ...
                         , [Chain
                              ( 9
                              , XXXreg
                              , [Chain (24, XXXoperand, [Chain (7, XXXstm, [])])]
                              )]
                         )]
                    )
                ]
    ...
    fun rewriterule (r: rule) =
      case r of _ => false
    fun getreplacement (XXXrewrite t) = t
      | getreplacement _ = raise InternalError "problem with rewrite 996"
  end
