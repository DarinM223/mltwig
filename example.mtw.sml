  ...
  struct
    ...
    val showSymbol =
      fn ARC t0 => "ARC " ^ "(" ^ Int.toString t0 ^ ")"
       ...
       | Mul => "Mul"
       | Minus => "Minus"
       | Plus (* The type and function definitions                   *) =>
        "Plus"
    ...
    val showInstr =
      fn PUSH t0 => "PUSH " ^ "(" ^ Int.toString t0 ^ ")"
       | PLUS => "PLUS"
       | MINUS => "MINUS"
       | MUL => "MUL"
       | PLUSMUL => "PLUSMUL"
        ...
        case n of
          5 =>
            XXXrewrite
              (case (()) of
                 (_) =>
                   (Tree ((get_subtree (2, ir)), Mul, (get_subtree (1, ir)))))
        | 4 =>
            X_Expr
              (case
               ...
               of
                 (X_Expr Expr1, X_Expr Expr2, X_Expr Expr3) =>
                   (Expr1 @ Expr2 @ Expr3 @ [PLUSMUL])
               | _ => raise InternalError "S5")
        | 3 =>
            X_Expr
              (case (execute (nth (children, 0)), execute (nth (children, 1))) of
                 (X_Expr Expr1, X_Expr Expr2) => (Expr1 @ Expr2 @ [MUL])
               | _ => raise InternalError "S5")
        | 2 =>
            X_Expr
              (case (execute (nth (children, 0)), execute (nth (children, 1))) of
                 (X_Expr Expr1, X_Expr Expr2) => (Expr1 @ Expr2 @ [MINUS])
               | _ => raise InternalError "S5")
        | 1 =>
            X_Expr
              (case (execute (nth (children, 0)), execute (nth (children, 1))) of
                 (X_Expr Expr1, X_Expr Expr2) => (Expr1 @ Expr2 @ [PLUS])
               | _ => raise InternalError "S5")
        | 0 => X_Expr ([PUSH (constValue ir)])
        | _ => raise Match
      end

    val matchcounts = [(5, 3), (4, 3), (3, 2), (2, 2), (1, 2), (0, 1)]
    val matchtable =
      let val a = Array.array (6, 0)
