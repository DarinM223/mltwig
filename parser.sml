...
struct
  ...
  type stable =
    (string -> class) * int * (string * string) list * (string * int) list
           ...
           (case p' of
              Unknown =>
                if a = 0 then
                  ...
                  conflict
                    ("symbol " ^ s ^ " is a node symboll with arity "
                     ^ (int2str a))
            | Terminal a' =>
                if a = a' then p
...
struct
  ...
  fun error s =
    raise ParserError
      ("Error in Line " ^ (int2str (current_line ())) ^ " : " ^ s)

  local
    fun digit c =
      (#"0" <= c) andalso (c <= #"9")
    fun str2int' (a, c :: r) =
          if digit c then str2int' (a * 10 + ord c - ord #"0", r)
          else (a, c :: r)
      | str2int' r = r
  in
    fun str2int s =
  ...
  fun parsespecification (s, st) =
    ...
    | IDENTIFIER n =>
        let
          val replacement =
            if n = "TOPDOWN" orelse n = "REWRITE" then
              case lexer () of
                IDENTIFIER n' => n'
  ...
  and parse_insert (([], a, b, c), st) =
        error "insert may not precede prologue"
                ...
                INT i =>
                  (case lexer () of
                     RPAREN =>
                       (case lexer () of
                          COMMA =>
                            parse_node (s, declare_node (st, id, str2int i))
                        | SEMICOLON =>
                            parsespecification
                              (s, declare_node (st, id, str2int i))
                        | _ => error ", or ; expected")
                   | _ => error "')' expected")
    ...
    case lexer () of
      IDENTIFIER id =>
        (case lexer () of
           IDENTIFIER "of" =>
             let
               val (tyexpr, token) = readtype ()
             in
               case token of
                 OTHER "|" => parse_label (s, declare_label (st, id, tyexpr))
               | SEMICOLON =>
                   parsespecification (s, declare_label (st, id, tyexpr))
               | _ => error "label list"
             end
         | _ => error "label list")
    | _ => error "label list"
  ...
  and read_ml_semicolon' () =
    case lexerS () of
      LPAREN =>
        ("(" :: read_ml_semicolon' ()) @ (")" :: (read_ml_semicolon' ()))
    | RPAREN => []
    | EOF => error "premature EOF"
    | t => (token2str t) :: (read_ml_semicolon' ())

  and read_ml_semicolon () =
    if lexer () <> LPAREN then
      error "'(' expected"
    else
      let val r = read_ml_semicolon' ()
      in if lexer () <> SEMICOLON then error "missing semicolon" else r
      end
  ...
  and rulecode () =
    if lexer () <> LPAREN then error "'(' expected" else "( " :: (rulecode' ())

  and parse_restaction (rn, ty, nt, pattern, costcode, (a, l, b, c), st) =
    let
      val mlcode = Action (rulecode ())
    in
  ...
  and parse_rule (ty, nt, s as (a, l, b, c), st) =
    let
      val (pattern, token) = parse_pattern st
    ...
    case lexer () of
      IDENTIFIER id =>
        (case lexer () of
           LPAREN =>
             let
               val subtrees = parse_subtrees st
         ...
         | token =>
             let
               val p = check_symbol (st, id, Unknown)
             in
               ( case p of
  ...
  and parse_subtrees st =
    let
      val (pat, token) = parse_pattern st
    in
      case token of
        COMMA => let val rest = parse_subtrees st in (pat :: rest) end
      | RPAREN => [pat]
  ...
  and parse_structurename ((a, b, c, _), st) =
    case lexer () of
      IDENTIFIER s =>
        if lexer () = SEMICOLON then parsespecification ((a, b, c, s), st)
  ...
  and construct_resulttype st =
    let
      val (first, rest) =
        case (rev o get_labels) st of
          first :: rest => (first, rest)
        | _ => raise Fail "construct_resulttype: empty labels"
      and makeCon = (fn (n, t) => "X_" ^ n ^ " of " ^ t)
    in
      List.foldl (fn (a, b) => a ^ " | " ^ b) (makeCon first) (map makeCon rest)
    end
  ...
  and specification instream =
    ...
    in
      if b = nil orelse a = nil then
        error "Prologue and rules are obligatory"
      else
        ( a
        , b
        , c
